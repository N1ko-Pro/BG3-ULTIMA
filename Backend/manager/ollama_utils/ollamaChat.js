  ...
  const rawText = await new Promise((resolve, reject) => {
    ...
    const req = http.request({
      ...
      if (res.statusCode < 200 || res.statusCode >= 300) {
        ...
        res.on('end', () => reject(new Error(`OLLAMA_HTTP_${res.statusCode}: ${Buffer.concat(errChunks).toString().slice(0, 400)}`)));
        return;
      }
    ...
    if (timeoutMs > 0) {
      req.setTimeout(timeoutMs, () => req.destroy(new Error('OLLAMA_TIMEOUT')));
    }

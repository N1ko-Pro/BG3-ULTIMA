...
// ─────────────────────────────────────────────────────────────────────────────
//  UpdatePanel
//  Rich status card for Settings → General. Shows current version, last check
//  result, available version + release notes, download progress, and
//  action buttons (Check / Download / Install).
// ─────────────────────────────────────────────────────────────────────────────

export default function UpdatePanel({ autoUpdateEnabled = true, onAutoUpdateToggle }) {
  const t = useLocale();
  const { state, currentVersion, check, download, install } = useUpdater();

  const handleAutoUpdateToggle = () => {
    onAutoUpdateToggle?.(!autoUpdateEnabled);
  };

  const { accentColor, Icon, iconTint, statusText, subText } = useMemo(
  ...
  return (
    <div className="relative w-full rounded-2xl border border-white/[0.07] bg-surface-2/40 backdrop-blur-xl p-4 overflow-hidden">
      ...
      <div className="flex items-center gap-3.5">
        <div className={`self-start w-8 h-8 rounded-xl border flex items-center justify-center shrink-0 ${accentColor.bg} ${accentColor.border}`}>
        ...
        <div className="shrink-0 flex items-center gap-2">
          {(state.status === 'idle' || state.status === 'not-available' || isError) && (
            <button
              onClick={handleCheck}
              disabled={isChecking}
        ...
        <div className="flex items-center justify-between px-7">
          ...
          <button
            ...
            className={`relative w-11 h-6 rounded-full transition-colors duration-200 ${
              autoUpdateEnabled ? 'bg-emerald-500/20 border border-emerald-500/30' : 'bg-white/[0.04] border border-white/[0.08]'
            }`}
          >
            <div
...
function describeStatus(state, t, currentVersion) {
  const base = { accentColor: COLORS.neutral, Icon: RefreshCw, iconTint: 'text-zinc-300' };
  switch (state.status) {
    case 'checking':
      return {
        ...base,
        ...
        subText: currentVersion ? t.updates.currentVersion(currentVersion) : '',
      };
    case 'available':
      return {
        ...
        Icon: Download,
        iconTint: 'text-indigo-300',
        statusText: t.updates.available(state.version || '?'),
        subText: currentVersion ? t.updates.currentVersion(currentVersion) : '',
      };
    case 'download-progress':
      return {
        ...
        subText: state.version ? t.updates.downloadingVersion(state.version) : '',
      };
    case 'downloaded':
      return {
        ...
        statusText: t.updates.ready,
        subText: state.version ? t.updates.readyVersion(state.version) : '',
      };
    case 'not-available':
      return {
        accentColor: COLORS.emerald,
    ...
    default:
      return {
        ...
        Icon: RefreshCw,
        iconTint: 'text-zinc-300',
        statusText: t.updates.idle,
        subText: currentVersion ? t.updates.currentVersion(currentVersion) : '',
      };
  }
}

...
import { getCategoryById } from './DictionaryCategories.constants';
import { TagPicker } from './DictionaryCategories';
import { autoResize } from '../../../Utils/Miscell/autoResize';
      ...
      <div ref={rowRef} className="px-4 py-3 rounded-xl bg-surface-3/80 border border-white/[0.12] space-y-2">
        ...
        <div className="flex gap-2 items-start">
          ...
          <div className="flex gap-1 items-center shrink-0 pt-[3px]">
            ...
            <button onClick={save} className="w-7 h-7 flex items-center justify-center rounded-lg text-emerald-400 hover:bg-emerald-500/10 transition-colors">
  ...
  return (
    <div className="group grid grid-cols-[minmax(0,2fr)_1px_minmax(0,3fr)_56px] px-4 py-2.5 rounded-xl border border-transparent hover:bg-surface-2/60 hover:border-white/[0.06] items-start transition-all duration-150">
      ...
      <div className="flex gap-1 justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-150">
        <button onClick={startEdit} className="w-7 h-7 flex items-center justify-center rounded-lg text-zinc-500 hover:text-zinc-200 hover:bg-white/[0.06] transition-colors">
          <Pencil className="w-3.5 h-3.5" />
        </button>
        <button onClick={() => onDelete(entry.id)} className="w-7 h-7 flex items-center justify-center rounded-lg text-zinc-600 hover:text-red-400 hover:bg-red-500/[0.08] transition-colors">
...
export function AddRow({ onAdd, entries = [] }) {
  ...
  const isDuplicate = src.trim() && entries.some((e) => e.source === src.trim());
  ...
  return (
    <div ref={rowRef}>
      <div className={`grid grid-cols-[minmax(0,2fr)_1px_minmax(0,3fr)_auto] px-4 py-2.5 rounded-xl border transition-all duration-150 gap-x-3 ${isDuplicate ? 'border-red-500/30 bg-red-500/[0.04]' : active ? 'bg-surface-3/60 border-white/[0.1]' : 'border-dashed border-white/[0.08] hover:border-white/[0.14]'}`}>
        <input
          ref={srcRef}
          value={src}
        ...
        <div className="flex gap-1 justify-center items-center">
          ...
          <button
            ...
            className={`w-7 h-7 flex items-center justify-center rounded-lg transition-all duration-200 disabled:opacity-25 disabled:pointer-events-none ${
              src.trim() && tgt.trim() && !isDuplicate

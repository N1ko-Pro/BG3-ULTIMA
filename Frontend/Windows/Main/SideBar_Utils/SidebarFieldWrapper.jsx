...
export default function SidebarFieldWrapper({
  ...
  // Orange when: uuid not regenerated, OR field is actually empty
  const isOrange = isOriginalUuid || (!hasValue && !isUnknown);

  return (
    <div
      className={`group relative -mx-3 p-3 rounded-xl transition-all duration-200 ${
        isRequiredMissing
    ...
    >
      <div
        className={`absolute h-[calc(100%-16px)] w-[2px] left-[2px] top-[8px] rounded-full transition-all duration-400 ${
          isRequiredMissing

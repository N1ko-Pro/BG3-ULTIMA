...
import PackModal from '../../Shared/ui/modal/PackModal';
...
export default function TopBar({
  ...
  const auth = useAuth();
  const { canUseDictionary } = auth;
  const t = useLocale();

  const handlePackClick = () => {
    if (hasOriginalUuid) {
      notify.warning(
        t.editor.uuidWarning,
        t.editor.uuidWarningDesc,
        6000
      );
      return;
    }
    ...
    if (!result?.isValid) {
      if (isDictionaryOpen && onCloseDictionary) onCloseDictionary();
      notify.warning(
        t.editor.packRequired,
        t.editor.packRequiredDesc,
        6500
      );
      return;
    }
  ...
  const handleDictionaryClick = () => {
    if (!canUseDictionary) {
      notify.warning(t.editor.dictionaryBlockedTitle, t.editor.dictionaryBlocked, 4000);
      return;
    }
    onToggleDictionary();
  };
  ...
  return (
    <>
      <header className={`h-20 border-b border-white/[0.06] bg-surface-1/95 backdrop-blur-2xl flex items-center justify-between pr-8 shrink-0 relative z-30 transition-[padding-left] duration-[600ms] ease-[cubic-bezier(0.4,0,0.2,1)] ${isDictionaryOpen ? 'pl-0' : 'pl-8'}`} data-tutorial="editor-toolbar">
        {/* Background overlay — continues the sidebar gradient seamlessly */}
        ...
        <div className="flex-1 min-w-0 flex items-center gap-3">
          <ToolsGroup className={`transition-[margin] duration-[600ms] ease-[cubic-bezier(0.4,0,0.2,1)] ${isDictionaryOpen ? '-ml-4' : ''}`} data-tutorial="editor-tools">
            {/* Dictionary — slides away when panel opens */}
            <div
            ...
            {modData && (
              <button
                onClick={() => window.electronAPI.openModFolder()}
            ...
            {modData && onCloseProject && (
              <button
                onClick={() => hasUnsavedChanges ? setIsCloseConfirmOpen(true) : onCloseProject()}
      ...
      {/* Auto-translate panel — always mounted, animates via grid-template-rows */}
      <PackModal
      ...
      <UnsavedChangesModal
        ...
        onSaveAndClose={async () => { setIsCloseConfirmOpen(false); if (onSaveProject) await onSaveProject(); onCloseProject(); }}
      />
    </>
  );
}

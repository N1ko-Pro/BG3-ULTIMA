...
export default function TranslatePage({ method, onMethodChange }) {
  ...
  const visibleMethods = AUTO_TRANSLATION_METHODS_BY_MODEL[activeModel] || AUTO_TRANSLATION_METHODS_BY_MODEL[MODEL_X];

  // Look up localized text per method id
  const METHOD_LOCALE = {
    ...
    <div className="flex flex-col min-h-full animate-[fadeIn_220ms_ease-out]">
      ...
      <div>
        <div className="flex items-center gap-2 mb-3 px-0.5">
          ...
          <span className="text-[12px] font-bold tracking-widest text-zinc-500 uppercase">{t.settings.workMode}</span>
        </div>
          ...
          {AUTO_TRANSLATION_MODELS.map((model) => {
            ...
            return (
              <button
                ...
                className={`relative flex-1 flex items-center justify-center gap-2.5 rounded-xl py-2.5 px-3 transition-all duration-200 ${
                  isActive
              ...
              >
                ...
                <div className={`w-6 h-6 rounded-lg flex items-center justify-center transition-colors duration-200 ${
                  isActive ? 'bg-white/[0.10] text-white' : 'bg-white/[0.04] text-zinc-500'
                }`}>
      ...
      <div>
        <div className="flex items-center gap-2 mb-3 px-0.5">
          ...
          <span className="text-[12px] font-bold tracking-widest text-zinc-500 uppercase">{t.settings.modelMode}</span>
        </div>
          ...
          {visibleMethods.map((item) => {
            ...
            return (
              <button
                ...
                className={`relative w-full rounded-2xl border p-4 text-left transition-all duration-200 overflow-hidden flex items-start gap-3.5 ${
                  isActive
              ...
              >
                ...
                <div className={`mt-0.5 w-8 h-8 rounded-xl flex items-center justify-center shrink-0 transition-colors duration-200 ${
                  isActive ? `${item.bg} ${item.color}` : 'bg-white/[0.04] text-zinc-500'
                }`}>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap mb-1">
                    ...
                    <span className={`rounded-full px-2 py-0.5 text-[11px] font-bold uppercase tracking-wider border ${
                      isActive
